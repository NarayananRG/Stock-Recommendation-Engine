...
from live_paper.run_after_close import _build_candidates, _existing_run, _holding_observations, _initialize_runs, _run_with_clock, render_report, require_after_close
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with Stage5DLedger(db) as ledger:
            ...
            report = {"run_id": "RUN", "market_session_date": "2026-09-22",
                      "market_data_hash": "HASH", "candidate_input_hash": "HASH",
            ...
            check("fill_uses_lifecycle", fill["status"] == "CREATED" and ledger._effective_fill_quantity(rec["recommendation_id"]) == quantity)
            ...
            check("cancel_prevents_new_fill", raises("PENDING", lambda: record_action(ledger, "fill", rec["recommendation_id"], quantity=1, price="100")))
