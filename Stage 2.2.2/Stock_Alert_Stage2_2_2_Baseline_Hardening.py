...
METADATA_COLUMNS = {
    ...
    "STAGE21_CODE_HASH", "STAGE222_CODE_HASH", "DATA_CONTENT_HASH",
...
def create_frozen_snapshot(s221: Any, args: argparse.Namespace) -> None:
    ...
    manifest = s221.build_data_manifest(data_dir, tickers, config, object())
    manifest["stage"] = STAGE
    manifest["yfinance_version"] = environment_report()["packages"]["yfinance"]
    manifest.pop("manifest_sha256", None)
    manifest["manifest_sha256"] = canonical_hash(manifest)
    write_json(manifest_path, manifest)
    verify_frozen_manifest(data_dir, manifest_path, DEFAULT_UNIVERSE)
...
def point_in_time_audit(
    ...
    """Prefix-invariance audit of daily, weekly, RS and signal computations."""
    rows: List[Dict[str, Any]] = []
    ticker = next((item for item in ("TCS.NS", "INFY.NS", *tickers) if item in engine.features), None)
    if ticker is None:
        return False, pd.DataFrame(), "No prepared stock was available for point-in-time audit"
    raw = engine.raw_data[ticker]
    nifty = engine.raw_data["^NSEI"]
    full = engine.features[ticker]
    eligible = full.index[
        (full.index >= normalize_date("2013-01-01"))
        & full.index.isin(raw.index)
    ]
    chosen: List[pd.Timestamp] = []
    for weekday in (0, 3, 4):
        matches = eligible[eligible.weekday == weekday]
        if len(matches):
            for position in np.linspace(0, len(matches) - 1, 2, dtype=int):
                chosen.append(normalize_date(matches[position]))
    chosen = sorted(set(chosen))[:6]
    compare_columns = [
        "SMA20", "SMA50", "SMA200", "RSI", "ATR", "ADX", "VolumeAvg",
        "VolumeRatio", "ST", "STTrend", "RecentHigh20", "BreakoutHigh20",
        "WeeklyRSI", "WeeklySTTrend", "WeeklyST", "RS20", "RS60", "RS120",
    ]
    for cutoff in chosen:
        ...
        if cutoff not in prefix_features.index or cutoff not in full.index:
            rows.append({"Ticker": ticker, "Date": cutoff, "Check": "PREFIX_ROW", "Status": "FAIL", "Detail": "cutoff missing"})
            continue
        for column in compare_columns:
            left = prefix_features.loc[cutoff, column]
            right = full.loc[cutoff, column]
            if pd.isna(left) and pd.isna(right):
                passed, difference = True, 0.0
            elif isinstance(left, str) or isinstance(right, str):
                passed, difference = str(left) == str(right), np.nan
            else:
                difference = abs(float(left) - float(right))
                passed = bool(np.isclose(left, right, atol=1e-10, rtol=1e-10, equal_nan=True))
            rows.append({
                "Ticker": ticker, "Date": cutoff, "Check": column,
                "Status": "PASS" if passed else "FAIL", "Absolute Difference": difference,
                "Prefix Value": left, "Full Value": right,
            })
        prefix_signal = strategy.signal_at(ticker, prefix_features, len(prefix_features) - 1)
        ...
        signal_pass = canonical_hash(prefix_signal) == canonical_hash(full_signal)
        rows.append({
            "Ticker": ticker, "Date": cutoff, "Check": "SIGNAL_OUTPUT",
            "Status": "PASS" if signal_pass else "FAIL",
            "Prefix Value": canonical_hash(prefix_signal),
            "Full Value": canonical_hash(full_signal),
        })
    audit = pd.DataFrame(rows)
    passed = bool(not audit.empty and audit["Status"].eq("PASS").all())
    ...
    for name, ok in static_checks.items():
        audit.loc[len(audit)] = {
            "Ticker": "SOURCE", "Date": pd.NaT, "Check": name,
            "Status": "PASS" if ok else "FAIL", "Absolute Difference": np.nan,
            "Prefix Value": "", "Full Value": "",
        }
    ...
    for row in merged.to_dict("records"):
        ...
        if row["_merge"] != "both":
            output.append({
                ...
                "Stage 2.2.2 Value": "PRESENT" if row["_merge"] == "left_only" else "MISSING",
                "Stage 2.2.1 Value": "PRESENT" if row["_merge"] == "right_only" else "MISSING",
        ...
        for column in wanted:
            ...
            if not equal:
                output.append({
                    ...
                    "Stage 2.2.2 Value": left_value, "Stage 2.2.1 Value": right_value,
                    "Absolute Difference": difference,
                })
    return pd.DataFrame(output)


def load_stage221_result(results_dir: Path, filename: str) -> pd.DataFrame:
    ...
    if not path.exists() and path.suffix == ".csv":
        ...
        if compressed.exists():
            path = compressed
    if not path.exists():
        raise FileNotFoundError(f"Stage 2.2.1 acceptance file missing: {path}")
    return pd.read_csv(path, low_memory=False)


def stage221_acceptance_parity(
    ...
    base_candidates = load_stage221_result(baseline_dir, "stage2_2_1_candidate_signal_log.csv")
    candidate_columns = [
    ...
    base_orders = load_stage221_result(baseline_dir, "stage2_2_1_order_log.csv")
    diffs.append(dataframe_differences("ORDER", orders, base_orders, ["Variant", "Order ID"]))
    base_trades = load_stage221_result(baseline_dir, "stage2_2_1_portfolio_trade_log.csv")
    ...
    base_equity = pd.concat([
        load_stage221_result(baseline_dir, f"stage2_2_1_daily_equity_{item['variant']}.csv")
        for item in results
    ], ignore_index=True)
    diffs.append(dataframe_differences("DAILY_EQUITY", equity, base_equity, ["Variant", "Date"]))
    base_summary = load_stage221_result(baseline_dir, "stage2_2_1_portfolio_summary.csv")
    ...
    material = pd.concat([item for item in diffs if not item.empty], ignore_index=True) if any(not item.empty for item in diffs) else pd.DataFrame(
        columns=["Category", "Key", "Field", "Stage 2.2.2 Value", "Stage 2.2.1 Value", "Absolute Difference"]
    )
    status = pd.DataFrame([
        {
...
def build_exposure_matched_benchmarks(
    ...
    for item in results:
        ...
        for exposure_method, allocation in (
            ("CONSTANT_AVERAGE_EXPOSURE", pd.Series(average_exposure, index=dates)),
            ("PRIOR_SESSION_DYNAMIC_EXPOSURE", prior_exposure),
        ):
            for cash_name, cash_return in cash_returns.items():
                ...
                rows.append({
                    ...
                    **metrics,
                    ...
                    "Interpretation": "Diagnostic exposure control; does not establish causality",
                    "No-Lookahead Rule": "Dynamic allocation uses prior-session strategy exposure",
                })
...
def validate_execution(
    ...
    reference_ok: bool,
    stage221_parity_ok: Optional[bool],
    ...
    add("Official loaded ticker set equals manifest", official_manifest_set_ok)
    ...
    if stage221_parity_ok is not None:
        add("Exact Stage 2.2.1 acceptance parity at 1.0x", stage221_parity_ok)
    ...
    report = "\n".join([
        ...
        *[f"{row['Check']:<58} {row['Status']:<5} {row['Detail']}".rstrip() for row in checks],
        "-" * 112, f"OVERALL VALIDATION: {label}",
        "STRATEGY RULES CHANGED: NO", "", SURVIVORSHIP_WARNING,
    ])
    return overall, pd.DataFrame(checks), report


def run_stage222_self_tests(s221: Any, stage21_path: Path) -> None:
    if sha256_file(stage21_path) != EXPECTED_STAGE21_HASH:
    ...
    portable_a = canonical_hash({"strategy": "abc", "data": "def"})
    portable_b = canonical_hash({"data": "def", "strategy": "abc"})
    assert portable_a == portable_b, "Canonical hash depends on ordering/path context"
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        try:
            verify_frozen_manifest(root, root / "manifest.json")
    ...
    test_equity = pd.DataFrame({
        "Date": dates,
        "Total Equity": [100.0, 101.0, 102.0],
        "Open Position Value": [50.0, 25.25, 0.0],
    })
    exposure = test_equity["Open Position Value"] / test_equity["Total Equity"]
    assert np.allclose(exposure, [0.5, 0.25, 0.0]), "Exposure math failed"
    assert np.allclose(exposure.shift(1).fillna(0.0), [0.0, 0.5, 0.25]), "Prior-session exposure rule failed"
    print("STAGE 2.2.2 SELF-TESTS: PASS")
...
def delivery_report(
    ...
    return f"""# Stage 2.2.2 Baseline Hardening Delivery Report

1. Overall status: **{status}**.
2. Run scope: {'TCS.NS / INFY.NS sanity' if sanity else 'official full universe'}; {config.test_start} through {config.test_end}.
3. Stage 2.1 hash gate: `{identity['STAGE21_CODE_HASH']}` (exact required hash).
4. Stage 2.2.2 code hash: `{identity['STAGE222_CODE_HASH']}`.
...
9. Portable Experiment ID: `{identity['experiment_id']}`; stable identity excludes absolute paths and environment versions.
...
12. Weekly behavior: {point_limitation}
...
16. Friction sensitivity: 0.5x, 1.0x, 1.5x, and 2.0x of baseline 5+5 BPS; 1.0x is official and signals are not regenerated. Stage 2.2.1 acceptance parity: **{parity_text}**.
17. Environment: Python {environment['python']}, pandas {environment['packages']['pandas']}, NumPy {environment['packages']['numpy']}, yfinance {environment['packages']['yfinance']}. Runtime {runtime_seconds:.3f} seconds. {SURVIVORSHIP_WARNING}
...
def save_outputs(
    ...
    tables: Dict[str, pd.DataFrame] = {
        ...
        f"{PREFIX}_stage221_acceptance_parity_report.csv": parity_differences,
        f"{PREFIX}_stage221_acceptance_parity_summary.csv": parity_status,
    ...
    text_files = {
        ...
        "Stage2_2_2_Delivery_Report.md": report,
...
def run_pipeline(args: argparse.Namespace, s221: Any) -> bool:
    ...
    data_dir = Path(args.frozen_data_dir)
    manifest_path = Path(args.manifest_path) if args.manifest_path else data_dir / "manifest.json"
    stage221_results = Path(args.stage221_results)
    ...
    if sanity:
        ...
        validate_sanity_receipt(args, stage21_before, stage222_before, data_hash)
    ...
    identity = identity_payloads(
        ...
        stage21_before, stage222_before, tickers,
    )
    for field in (
        "experiment_id", "STRATEGY_HASH", "EXECUTION_HASH", "STAGE21_CODE_HASH",
        "STAGE222_CODE_HASH", "DATA_CONTENT_HASH", "MANIFEST_DOCUMENT_HASH",
    ):
    ...
    parity_status = pd.DataFrame()
    if not sanity:
        parity_ok, parity_differences, parity_status = stage221_acceptance_parity(
            candidates, orders, trades, results, research["portfolio_summary"], stage221_results
        )
    ...
    overall, checks, validation_report = validate_execution(
        ...
        stage222_before, stage222_after, pit_ok, reference_ok, parity_ok, cost_table,
    )
    runtime_seconds = time.perf_counter() - started
    report = delivery_report(
        identity, config, overall, sanity, runtime_seconds, parity_status,
        pit_limitation, environment,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    baseline_dir = script_dir / "baseline" / "stage2_1"
    stage221_root = script_dir / "stage2_2_1"
    stage222_root = script_dir / "stage2_2_2"
    ...
    parser.add_argument("--stage221-source", default=str(script_dir / "Stock_Alert_Stage2_2_1_Reproducible_Benchmark.py"))
    ...
    parser.add_argument("--stage221-results", default=str(stage221_root / "results"))
...
def main() -> int:
    ...
    if args.create_frozen_snapshot:
        create_frozen_snapshot(s221, args)
        return 0
    if args.self_test:
        run_stage222_self_tests(s221, Path(args.stage21_source))
        return 0
    if args.sanity:
