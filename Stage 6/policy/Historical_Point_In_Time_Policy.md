...
Publication, observation, and retrieval timestamps remain separate. Missing timestamps stay missing and restrict eligibility; they are not silently replaced by one another. Corrections and revisions are versioned with their actual availability times. Raw evidence should be immutable and content-hashed where practical. Derived records must retain input IDs, hashes, versions, and the as-of cutoff so eligibility can be reproduced.
