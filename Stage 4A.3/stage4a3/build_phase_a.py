...
def build(repo: Path,stage_root: Path,output_root: Path) -> dict[str,Any]:
    ...
    validation=pd.DataFrame([
        ...
        {"Check":"all 164 unit and hardening tests","Status":"PASS" if all(pd.read_csv(path)["Status"].eq("PASS").all() for path in (stage_root/"results/stage4a3_unit_test_results.csv",stage_root/"results/stage4a3a_hardening_test_results.csv",stage_root/"results/stage4a3_economic_methodology_test_results.csv")) else "FAIL","Details":"118 original + 26 prior hardening + 20 economic/session hardening"},
