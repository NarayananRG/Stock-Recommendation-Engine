...
def build(repo: Path,stage_root: Path,output_root: Path) -> dict[str,Any]:
    ...
    hash_spec={"version":"STAGE4A3_HASH_CHAIN_V2","snapshot_content":"canonical metadata + prediction logical hash + feature logical hash + market manifest + candidate input manifest","genesis_formula":"SHA256(protocol_tag_commit + model_bundle_hash + 'STAGE4A3_GENESIS')","current_formula":"SHA256(previous_chain_hash + signal_date + snapshot_content_hash + protocol_commit + model_bundle_hash)","index":"append-only, strictly increasing Signal Date","session_coverage":"separate append-only chain; CAPTURED or MISSED; no backfill"}
    gate_spec={"status":"LOCKED_UNTIL_ALL_PASS","conditions":config["final_gate"],"force_flags_allowed":False,"final_outputs_locked":True}
    hypothesis={"primary_confirmatory_policy":"R3_K1","definition":"TRANSFER JOINT_T1 LOGIT_FULL, K=1","primary_comparator":"R0_K1","secondary_substitution_allowed":False,"stage5_requires_all_ten":True,"economic_criteria":config["final_economic_criteria"],"bootstrap":config["bootstrap"],"random_control":config["random_control"],"portfolio_window":config["final_portfolio_window"],"market_data":config["final_market_data"]}
    ...
    validation=pd.DataFrame([
        ...
        {"Check":"all 188 unit and hardening tests","Status":"PASS" if all(pd.read_csv(path)["Status"].eq("PASS").all() for path in (stage_root/"results/stage4a3_unit_test_results.csv",stage_root/"results/stage4a3a_hardening_test_results.csv",stage_root/"results/stage4a3_economic_methodology_test_results.csv",stage_root/"results/stage4a3_freeze_readiness_test_results.csv")) else "FAIL","Details":"118 original + 26 prior hardening + 20 economic/session hardening + 24 freeze-readiness"},
        {"Check":"true end-to-end replay acceptance","Status":"PASS" if all(pd.read_csv(stage_root/"results"/name)["Status"].isin(["PASS_EXACT","PASS_SERIALIZATION_ONLY"]).all() for name in ["stage4a3_end_to_end_replay_summary_parity.csv","stage4a3_end_to_end_replay_trade_parity.csv","stage4a3_end_to_end_replay_daily_parity.csv","stage4a3_end_to_end_replay_d0_parity.csv","stage4a3_end_to_end_replay_bootstrap_parity.csv","stage4a3_end_to_end_replay_random_parity.csv"]) else "FAIL","Details":"R0_K1/R3_K1 D1, D0, bootstrap and 500 controls through current adapter"},
