...
def write_snapshot(snapshot_root: Path, audit_root: Path, signal_date: str, metadata: dict[str,Any], predictions: pd.DataFrame, features: pd.DataFrame, market_manifest: dict[str,Any], previous_chain_hash: str, protocol_commit: str, model_bundle_hash: str, git_commit: str="", candidate_input_manifest: dict[str,Any]|None=None) -> dict[str,Any]:
    ...
    content_hash=snapshot_content_hash(metadata,predictions,features,market_manifest,candidate_input_manifest)
    chain=current_chain_hash(previous_chain_hash,signal_date,content_hash,protocol_commit,model_bundle_hash)
    target.mkdir(parents=True,exist_ok=False)
    _json(metadata,target/"snapshot_metadata.json");_csv_gz(predictions,target/"candidate_predictions.csv.gz");_csv_gz(features,target/"feature_snapshot.csv.gz");_json(market_manifest,target/"market_data_manifest.json");_json(candidate_input_manifest or {},target/"candidate_input_manifest.json")
    files=["snapshot_metadata.json","candidate_predictions.csv.gz","feature_snapshot.csv.gz","market_data_manifest.json","candidate_input_manifest.json"]
