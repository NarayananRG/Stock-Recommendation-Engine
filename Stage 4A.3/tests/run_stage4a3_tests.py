...
from stage4a3.outcome_resolver import build_event,verify_event_chain
...
def evaluate() -> list[dict]:
    ...
    fixture=pd.read_csv(ROOT/"tests/fixtures/HISTORICAL_TEST_FIXTURE_2025-12-10.csv.gz");pred=pd.read_csv(ROOT/"tests/dry_run/snapshots/2025/2025-12-10/candidate_predictions.csv.gz")
    ...
    snapdir=ROOT/"tests/dry_run/snapshots/2025/2025-12-10";meta=json.loads((snapdir/"snapshot_metadata.json").read_text());values[56]=canonical_json_hash(meta)==canonical_json_hash(meta);g=genesis_hash("p","m");values[57]=g==genesis_hash("p","m");ch=current_chain_hash(g,"2025-01-01","s","p","m");values[58]=ch==current_chain_hash(g,"2025-01-01","s","p","m")
    with tempfile.TemporaryDirectory() as td:
        temp=Path(td);shutil.copytree(ROOT/"tests/dry_run/snapshots",temp/"snapshots");shutil.copytree(ROOT/"tests/dry_run/audit",temp/"audit");idx=pd.read_csv(temp/"audit/prospective_snapshot_index.csv",dtype=str);pc=idx.iloc[0]["Previous Chain Hash"]
        ...
        shutil.rmtree(temp/"snapshots");shutil.copytree(ROOT/"tests/dry_run/snapshots",temp/"snapshots");(temp/"snapshots/2025/2025-12-10/candidate_predictions.csv.gz").unlink();values[60]=not valid()
    bad=pd.DataFrame([{c:"" for c in INDEX_COLUMNS} for _ in range(2)]);bad["Sequence"]=[1,2];bad["Signal Date"]=["2025-02-01","2025-01-01"];values[61]=not verify_index(bad,"p","m",g);bad["Signal Date"]=["2025-01-01"]*2;values[62]=not verify_index(bad,"p","m",g);values[63]="append-only" in json.dumps(SNAPSHOT_SCHEMA).lower() or "NO_OVERWRITE_NO_APPEND" in json.dumps(SNAPSHOT_SCHEMA);values[64]="PROTOCOL_CODE_DIRTY" in (ROOT/"stage4a3/activation.py").read_text();values[65]=values[22] and values[23]
    tag=config["protocol_tag"];values[66]=subprocess.call(["git","show-ref","--verify","--quiet",f"refs/tags/{tag}"],cwd=REPO)!=0;act_source=(ROOT/"stage4a3/activation.py").read_text();values[67]="ACTIVATION_RECORD_IMMUTABLE" in act_source;activation={"Activation UTC":"2026-01-01T10:00:00+00:00"};values[68]=not signal_is_after_activation(activation,"2026-01-01T09:59:59+00:00","2026-01-01");values[69]=signal_is_after_activation(activation,"2026-01-01T10:00:01+00:00","2026-01-01");values[70]="record_path.exists()" in act_source
