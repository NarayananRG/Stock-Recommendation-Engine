...
def compare_frames(name: str, actual: pd.DataFrame, expected: pd.DataFrame, keys: List[str]) -> tuple[int, pd.DataFrame]:
    meta = {c for c in expected.columns if c in {"Experiment ID","STRATEGY_HASH","EXECUTION_HASH","STAGE21_CODE_HASH","STAGE221_CODE_HASH","STAGE222_CODE_HASH","DATA_CONTENT_HASH","MANIFEST_DOCUMENT_HASH","Survivorship Bias Warning"}}
